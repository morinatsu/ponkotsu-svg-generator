...
describe('useInteractionManager', () => {
  let dispatch: ReturnType<typeof vi.fn> & React.Dispatch<Action>;
  let state: AppState;
  ...
  beforeEach(() => {
    dispatch = vi.fn() as unknown as ReturnType<typeof vi.fn> & React.Dispatch<Action>;
    state = {
      canvasWidth: 800,
      canvasHeight: 600,
  ...
  it('should attach mousemove listener to SVG when mode is idle', () => {
    renderHook(() => useInteractionManager(dispatch, state, svgRef, wasDraggedRef));
  ...
  it('should cleanup listeners on unmount', () => {
    const { unmount } = renderHook(() =>
      useInteractionManager(dispatch, state, svgRef, wasDraggedRef),
    );
    unmount();
  ...
  it('should handle idle mouse move (check resize handle)', () => {
    ...
    state.shapes = [
      { id: '1', type: 'rectangle', x: 0, y: 0, width: 100, height: 100, rotation: 0 },
    ];
