...
### 5.1. 開発サーバーを必要とするテスト (E2Eテスト等)

**重要:** AIエージェント（Jules）の実行環境では、リソース制約やセキュリティ上の理由により、`pnpm run dev &` のような**バックグラウンドでのサーバー起動プロセスは禁止されていると想定します。** (内部エラーが発生し、処理が停止する問題がしばしば発生しています)

開発サーバーの起動を必要とするテスト（E2Eテストなど）を実行する場合は、必ず `start-server-and-test` ユーティリティを経由する **専用の `scripts`** を使用してください。

**実行方法:**

```Bash
# サーバーの自動起動、E2Eテスト実行、サーバーの自動停止をすべて行う
